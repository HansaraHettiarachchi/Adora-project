...
export class StockService {
    async deleteBatchImages(batch_id) {
    ...
    async generateUniqueCode(baseCode) {
        let code = baseCode;
        let exists = true;
        let attempt = 0;
        while (exists) {
    ...
    async setStock(data, imageFiles) {
        let batch;
        ...
        if (imageFiles && imageFiles.length > 0) {
            ...
            for (const file of imageFiles) {
                ...
                const imageUrl = await FileUploader.uploadFile(file, path.join('batch', String(batch.id), imageName));
